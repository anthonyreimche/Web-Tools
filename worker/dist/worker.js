...
var json = (o, status = 200) => new Response(JSON.stringify(o), { status, headers: { "Content-Type": "application/json", ...CORS } });
var writeOk = (req, env) => {
...
var src_default = {
  async fetch(req, env) {
    ...
    if (req.method === "POST" && p[0] === "submit") {
      ...
      const decision = {
        schema: "swt.decision/1",
        projectId,
        submittedAt: (/* @__PURE__ */ new Date()).toISOString(),
        client: body.client || {},
        note: typeof body.note === "string" ? body.note.slice(0, 2e3) : "",
...
<div class="lb hide" id=lb><div class=lbtop><span class=cnt id=lcnt></span><span class=fn id=lfn></span><button class=iconbtn id=x aria-label=Close>\u2715</button></div><div class=lbstage id=lstage><button class="nav pv" id=pv aria-label=Previous>\u2039</button><img id=lbi alt=""><button class="nav nx" id=nx aria-label=Next>\u203A</button><div class="cue pick" id=cuePick><span class=ic>\u2713</span>Pick</div><div class="cue reject" id=cueReject><span class=ic>\u2717</span>Reject</div></div><div class=lbbtm><div class=lhint id=lhint>Swipe \u2191 Pick \xB7 \u2193 Reject \xB7 \u2190 \u2192 Browse</div><div class=lacts id=lacts><button class=lp id=lp><span>\u2713</span> Pick</button><button class=lr id=lr><span>\u2717</span> Reject</button></div></div></div>
<script>
...
fetch("/pj/"+ID).then(function(r){return r.json()}).then(function(p){P=p;pub=p.kind==="public";
 ...
 $("lhint").textContent=pub?"Swipe \u2190 \u2192 to browse":"Swipe \u2191 Pick \xB7 \u2193 Reject \xB7 \u2190 \u2192 Browse";
